...
import { selectCharacter, conditionToggled } from '../../../store/slices/characterSlice';
import { Zap, RotateCcw, User, Skull, ChevronRight, Activity } from 'lucide-react';
...
export const InitiativeTracker: React.FC = () => {
    ...
    const [managingConditionsFor, setManagingConditionsFor] = useState<string | null>(null);
    ...
    const getActorConditions = (id: string) => {
        if (id === 'player') return character.conditions || [];
        const minion = minions.find(m => m.id === id);
        return minion?.conditions || [];
    };

    const handleToggleCondition = (condition: string) => {
        if (!managingConditionsFor) return;

        if (managingConditionsFor === 'player') {
            dispatch(conditionToggled(condition));
        } else {
            dispatch(minionConditionToggled({ id: managingConditionsFor, condition }));
        }
    };
    ...
    return (
        <>
                ...
                <div className="p-4 space-y-2 max-h-64 overflow-y-auto custom-scrollbar">
                    {initiativeOrder.length === 0 ? (
                        ...
                        initiativeOrder.map((id, index) => {
                            ...
                            const conditions = getActorConditions(id);
                            ...
                            return (
                                <div
                                    ...
                                    ${isActive
                                            ? 'bg-accent/10 border-accent shadow-[0_0_20px_rgba(212,177,58,0.25)] scale-[1.03] z-10 opacity-100'
                                            : 'bg-black/20 border-white/5 opacity-40 hover:opacity-100 hover:border-white/20 blur-[0.5px] hover:blur-0 scale-95 hover:scale-100'
                                        }
                                `}
                                >
                                    <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {conditions.length > 0 && (
                                                    <div className="flex gap-1">
                                                        {conditions.map(c => (
                                                            <div key={c} className="w-1.5 h-1.5 rounded-full bg-red-400" title={c} />
                                                        ))}
                                                    </div>
                                                )}
                                            </div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                setManagingConditionsFor(id);
                                            }}
                                            className={`
                                            p-1.5 rounded-md transition-colors
                                            ${conditions.length > 0 ? 'text-red-400 bg-red-400/10' : 'text-muted hover:text-white'}
                                        `}
            ...
            <ConditionManager
                isOpen={!!managingConditionsFor}
                onClose={() => setManagingConditionsFor(null)}
                actorName={managingConditionsFor ? getActorName(managingConditionsFor) : ''}
                activeConditions={managingConditionsFor ? getActorConditions(managingConditionsFor) : []}
